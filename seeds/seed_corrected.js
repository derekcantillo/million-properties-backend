// Seed corregido para MillionPropertiesDB
// Implementa el patrón de referencias con colecciones separadas
...
// 1. CREAR COLECCIÓN DE OWNERS
print("Creando colección de Owners...");
const owners = [
  {
    _id: ObjectId(),
    name: "Owner 1",
...
// 2. CREAR COLECCIÓN DE PROPERTIES
print("Creando colección de Properties...");
const properties = [
  {
    _id: ObjectId(),
...
print(`✅ Insertadas ${properties.length} properties`);

// 3. CREAR COLECCIÓN DE PROPERTY IMAGES
print("Creando colección de PropertyImages...");
const propertyImages = [];

// Generar imágenes para cada propiedad (2-4 imágenes por propiedad)
properties.forEach((property, index) => {
  ...
  for (let i = 0; i < numImages; i++) {
    propertyImages.push({
      ...
      enabled: Math.random() > 0.3 // 70% habilitadas
    });
  }
});

db.PropertyImages.insertMany(propertyImages);
print(`✅ Insertadas ${propertyImages.length} property images`);

// 4. CREAR COLECCIÓN DE PROPERTY TRACES
print("Creando colección de PropertyTraces...");
const propertyTraces = [];

// Generar trazas para algunas propiedades (no todas tienen historial)
const propertiesWithTraces = properties.filter((_, index) => index % 3 === 0); // 1 de cada 3 propiedades

propertiesWithTraces.forEach((property, index) => {
  const numTraces = Math.floor(Math.random() * 3) + 1; // 1-3 trazas
...
print(`✅ Insertadas ${propertyTraces.length} property traces`);
...
print("La base de datos ahora tiene la estructura correcta con colecciones separadas y referencias.");